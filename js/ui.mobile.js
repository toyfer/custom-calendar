...
function eventYmd(e) {
  try {
    if (e.allDay && e.start && String(e.start).length >= 10) {
...
// Swipe handlers attached once per grid element
...
export function renderMonthGrid(state, { onSelectDate, onDrop, onSwipeMonth }) {
    ...
    const dotsHtml =
      evs
        ...
        .map((e) => {
          ...
          return `<i class="dot ${e.allDay ? 'allday' : ''}" style="--c:${col}"></i>`;
        })
  ...
  // Bind swipe once (not on every paint)
...
export function renderDayDrawer(state, { onEdit, onDelete, onCreate }) {
  ...
  for (const e of evs) {
    ...
    let timeLabel = '';
    if (e.allDay) {
      timeLabel = '<span class="badge">終日</span>';
    } else {
      try {
    ...
    row.innerHTML = `
      <div class="e-time">${timeLabel}</div>
      <div class="e-main">
        <div class="e-title"></div>
        <div class="e-meta"></div>
      </div>
      <div class="e-dot" style="background:${a ? a.color : '#5B6CFF'}"></div>
      <div class="e-actions">
        <button type="button" class="sm ghost" data-edit>編集</button>
        <button type="button" class="sm ghost" data-del>削除</button>
      </div>`;
    row.querySelector('.e-title').textContent = e.summary || '(無題)';
    row.querySelector('.e-meta').textContent = (a ? a.email : '') + loc;
...
export function renderAcctSheet(state, { onToggle }) {
  ...
  for (const a of state.accounts) {
    ...
    row.innerHTML = `
      ...
      <input type="checkbox" class="toggle" ${a.visible !== false ? 'checked' : ''}>
    `;
    row.querySelector('b').textContent = a.name || a.email || 'account';
    row.querySelector('small').textContent = a.email + (a.stale ? ' · 要再連携' : '');
