...
function ensureFlatpickr(){
  ...
  const common={ disableMobile:true, allowInput:false, locale: (window.flatpickr && flatpickr.l10ns && flatpickr.l10ns.ja) ? 'ja' : undefined, time_24hr:true, minuteIncrement:5 };
  if(fpStart) try{fpStart.destroy();}catch{}
  if(fpEnd) try{fpEnd.destroy();}catch{}
  if(isAllDay){
    fpStart=flatpickr(startEl, { ...common, enableTime:false, dateFormat:'Y-m-d', altInput:false });
    fpEnd=flatpickr(endEl, { ...common, enableTime:false, dateFormat:'Y-m-d', altInput:false });
  } else {
    fpStart=flatpickr(startEl, { ...common, enableTime:true, dateFormat:'Y-m-d H:i', altInput:false });
    fpEnd=flatpickr(endEl, { ...common, enableTime:true, dateFormat:'Y-m-d H:i', altInput:false });
  }
}
...
function openComposer(mode, ev=null, ymd=null){
  ...
  const summary=$('composerSummary'); const allDay=$('composerAllDay'); const start=$('composerStart'); const end=$('composerEnd'); const loc=$('composerLocation'); const desc=$('composerDesc'); const err=$('composerError');
  ...
  if(mode==='edit' && ev){
    ...
    setTimeout(()=>{
      ensureFlatpickr();
      try{
        if(ev.allDay){ if(fpStart) fpStart.setDate((ev.start||'').slice(0,10), true); if(fpEnd) fpEnd.setDate((ev.end||'').slice(0,10), true); }
...
async function onComposerSave(){
  if(!validateComposer()){ const f=$('composerForm'); if(f) f.reportValidity(); return; }
...
function wire(){
  ...
  $('composerCalendarBtn')?.addEventListener('click', ()=>{
    ...
    ui.renderComposerCalendarList(state, accId, (calId)=>{ const h=$('composerCalendar'); if(h) h.value=calId; ui.fillComposerCalendarBtn(state, calId); $('composerCalendarSheet').hidden=true; });
