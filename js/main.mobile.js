  ...
  const target =
    id === null
      ...
      : id ||
        (state.soloAccountId
          ? null
          : state.accounts.find((a) => a.visible !== false)?.id || null);
...
function openComposer(mode, ev = null, ymd = null) {
  ...
  let accountId =
    state.createAccountId || liveAccounts(state)[0]?.id || state.accounts[0]?.id || '';
...
async function onComposerSave() {
  ...
  if (!acct || acct.stale) {
    ui.toast('有効なアカウントを選んでください', 'error');
    return;
  }
...
async function fetchAll(opts = {}) {
  ...
  const run = (async () => {
    ...
    if (!state.online) {
      ...
      paint();
      ui.toast(
        hadCache ? 'オフライン · キャッシュ表示' : 'オフライン · キャッシュなし',
        hadCache ? 'ok' : 'error',
      );
      return;
    }
