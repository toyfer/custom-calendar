/** main.mobile — reauth, delete sheet, theme, view-first drawer */
...
function staleCount() {
  return (state.accounts || []).filter((a) => a.stale).length;
}
function updateConnectivity() {
  markStaleFlags(state);
  ui.setStatusDot(state.online, !!state.fromCache, { staleCount: staleCount() });
  document.body.classList.toggle('is-offline', !state.online);
  document.body.classList.toggle('has-stale', staleCount() > 0);
}
...
function paint() {
  ...
  ui.renderAcctSheet(state, {
    onToggle: (id, visible) => {
      ...
      if (!a) return;
      if (state.soloAccountId) {
        state.soloAccountId = null;
        state.accounts.forEach((x) => (x.visible = true));
      }
      a.visible = visible;
    ...
    onSolo: (id) => toggleSolo(id),
    onReauth: (id) => reauth(id),
  });
  ui.setChromeVisibility(state);
  ui.syncThemeUI(getStoredTheme());
}
...
function toggleSolo(id) {
  const target =
    id === null
      ? null
      : id || (state.soloAccountId ? null : state.accounts.find((a) => a.visible !== false)?.id || null);
...
async function reauth(accountId) {
  ...
  const targets = accountId
    ? [accountById(state, accountId)].filter(Boolean)
    : state.accounts.filter((a) => a.stale);
  if (!targets.length) {
    // still allow reauth all accounts
    if (accountId === null) {
      targets.push(...state.accounts);
    }
  }
  if (!targets.length) {
    ui.toast('再連携するアカウントがありません');
    return;
  }

  reauthBusy = true;
  const bannerBtn = $('sessionReauthBtn');
  ...
  try {
    ...
    for (const acc of targets) {
      try {
        // silent first
        const silent = await trySilentRefresh(state, acc.id);
        if (silent) {
          acc.stale = false;
          ok += 1;
          continue;
        }
        await connectAccount(state, { mode: 'reauth', hintEmail: acc.email });
        acc.stale = false;
        ok += 1;
      } catch (err) {
        console.error('reauth failed', acc.email, err);
        acc.stale = true;
        ui.toast(`${acc.email} の再連携に失敗`, 'error');
      }
    }
    persistAccounts(state);
    markStaleFlags(state);
...
function ensureFlatpickr() {
  const startEl = $('composerStart');
  const endEl = $('composerEnd');
...
function openComposer(mode, ev = null, ymd = null) {
  ...
  const allDay = $('composerAllDay');
  const loc = $('composerLocation');
  const desc = $('composerDesc');
  ...
  if (mode === 'edit' && ev) {
    ...
    ui.fillComposerAccountBtn(state, accountId);
    ui.fillComposerCalendarBtn(state, calendarId);
    setTimeout(() => {
      ensureFlatpickr();
      try {
        if (ev.allDay) {
          if (fpStart) fpStart.setDate(String(ev.start || '').slice(0, 10), true);
          if (fpEnd) fpEnd.setDate(String(ev.end || '').slice(0, 10), true);
        } else {
          if (fpStart) fpStart.setDate(new Date(ev.start), true);
          if (fpEnd) fpEnd.setDate(new Date(ev.end), true);
        }
      } catch {}
    }, 30);
  } else {
    state.editingEvent = null;
    // prefer live (non-stale) account for create
    const live = liveAccounts(state)[0];
    if (live) accountId = live.id;
    ...
    setTimeout(() => {
      ensureFlatpickr();
      try {
        if (fpStart) fpStart.setDate(s, true);
        if (fpEnd) fpEnd.setDate(e, true);
      } catch {}
    }, 30);
  }
  const accHidden = $('composerAccount');
  if (accHidden) accHidden.value = accountId;
  const calHidden = $('composerCalendar');
  if (calHidden) calHidden.value = calendarId;
  sheet.hidden = false;
...
function validateComposer() {
  ...
  if (summary && !summary.value.trim()) {
    ok = false;
    msg = 'タイトルを入力してください';
    summary.setCustomValidity(msg);
  } else if (summary) summary.setCustomValidity('');
  if (s && !s.value) {
    ok = false;
    msg = msg || '開始を入力してください';
    s.setCustomValidity(msg);
  } else if (s) s.setCustomValidity('');
  if (e && !e.value) {
    ok = false;
    msg = msg || '終了を入力してください';
    e.setCustomValidity(msg);
  } else if (e) e.setCustomValidity('');
  if (ok && s && e && s.value && e.value) {
    let sv, ev;
    try {
      sv = s._flatpickr ? s._flatpickr.selectedDates[0] : new Date(s.value);
      ev = e._flatpickr ? e._flatpickr.selectedDates[0] : new Date(e.value);
    } catch {
      sv = new Date(s.value);
      ev = new Date(e.value);
    }
...
async function onComposerSave() {
  ...
  const sEl = $('composerStart');
  const eEl = $('composerEnd');
  if (allDay) {
    startLocal = sEl.value.slice(0, 10);
    endLocal = eEl.value.slice(0, 10);
  } else {
    ...
    endLocal = toLocalInputValue(eDate);
  }
  if (!accountId) {
    ui.toast('アカウントを選んでください', 'error');
    return;
  }
  const acct = accountById(state, accountId);
  if (!acct || acct.stale) {
    ui.toast('再連携が必要です', 'error');
    openAcctSheet();
    return;
  }
  ...
  try {
    if (composerMode === 'edit' && state.editingEvent) {
      ...
      const needsMove = accountId !== origAccountId || calendarId !== origCalendarId;
      ...
      if (needsMove) {
        ...
        try {
          await apiDeleteEvent(state, origAccountId, cur.id, origCalendarId, {
            scope: 'single',
            recurringEventId: cur.recurringEventId,
          });
        } catch (delErr) {
          console.warn('move: delete original failed after insert', delErr);
          ui.toast('別カレンダーに複製しました（元は手動削除が必要）', 'error');
        }
        ui.toast(accountId !== origAccountId ? '別アカウントに移動しました' : '別カレンダーに移動しました', 'ok');
      } else {
        await patchEvent(state, cur.accountId, cur.id, resource, cur.calendarId || 'primary', {
          scope: 'single',
          recurringEventId: cur.recurringEventId,
        });
        ui.toast('更新しました', 'ok');
    ...
    console.error(err);
    const m = err?.data?.error?.message || err?.message || String(err);
    if (String(m).includes('401') || err?.status === 401) {
      if (acct) acct.stale = true;
      persistAccounts(state);
      ui.toast('セッション切れ — 再連携してください', 'error');
      paint();
      openAcctSheet();
    } else {
      ui.toast('保存失敗: ' + m, 'error');
    }
    const er = $('composerError');
    if (er) {
      er.textContent = '保存失敗: ' + m;
      er.hidden = false;
    }
  } finally {
...
function maybeEnableAuth() {
  ...
  paint();
  if (liveAccounts(state).length) fetchAll();
  else if (state.accounts.length && staleCount() > 0) {
    // accounts exist but all stale — surface banner
    paint();
  }
}
window.__gapiLoaded = () => {
...
async function fetchAll(opts = {}) {
  ...
  const run = (async () => {
    ...
    if (!targets.length) {
      paint();
      if (state.accounts.length) ui.toast('再連携が必要です', 'error');
      return;
    }
    ...
    for (const a of state.accounts) {
      ...
      if (tok?.accessToken && tok.expiresAt && tok.expiresAt < Date.now() + 120000)
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (r.status === 'fulfilled') {
          ...
          if (msg.includes('401') || r.reason?.status === 401 || msg.includes('token')) acc.stale = true;
        }
      }
      const other = state.events.filter((e) => e.monthKey && e.monthKey !== mk);
      ...
      if (fail) {
        ...
        for (const e of fromFailed) if (!okIds.has(e.uid)) state.events.push(e);
      }
      persistAccounts(state);
      state.fromCache = false;
      markStaleFlags(state);
      if (fail && !merged.length) ui.toast(`${fail}アカウント取得失敗 — 再連携してください`, 'error');
  ...
  ui.openDeleteSheet(ev, {
    onConfirm: async () => {
      try {
        ...
        if (String(m).includes('401') || err?.status === 401) {
          const a = accountById(state, ev.accountId);
          if (a) a.stale = true;
          persistAccounts(state);
          ui.toast('セッション切れ — 再連携してください', 'error');
          paint();
          openAcctSheet();
        } else ui.toast('削除失敗', 'error');
      }
    },
  });
}
...
async function handleDrop(uid, ymd) {
  ...
  try {
    await patchEvent(state, ev.accountId, ev.id, resource, ev.calendarId || 'primary', { scope: 'single' });
    ui.toast('移動しました', 'ok');
    await fetchAll({ force: true });
  } catch {
    ui.toast('移動失敗', 'error');
...
function wire() {
  ...
  $('drawerExpandBtn')?.addEventListener('click', () => {
    const d = $('dayDrawer');
    const cur = d?.getAttribute('data-detent') || 'peek';
    if (cur === 'peek') ui.setDrawerDetent('half');
    else if (cur === 'half') ui.setDrawerDetent('full');
  ...
  $('sessionBanner')?.querySelector('[data-close-session]')?.addEventListener('click', () => {
    const b = $('sessionBanner');
    if (b) b.hidden = true;
  });
  ...
  document.querySelectorAll('[data-theme-option]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      ui.syncThemeUI(mode);
      ui.toast(
        mode === 'system' ? 'システム設定に合わせます' : mode === 'dark' ? 'ダークテーマ' : 'ライトテーマ',
        'ok',
      );
    });
  });
  ...
  document.querySelectorAll('[data-close-ym]').forEach((el) =>
  ...
  $('composerAccountBtn')?.addEventListener('click', () => {
    ...
    ui.renderComposerAccountList(state, (id) => {
      ...
      ui.fillComposerAccountBtn(state, id);
      $('composerAccountSheet').hidden = true;
      const calList = state.calendarsByAccount[id] || [];
      const firstCal = calList[0]?.id || 'primary';
      const calHidden = $('composerCalendar');
      if (calHidden) calHidden.value = firstCal;
      ui.fillComposerCalendarBtn(state, firstCal);
    });
    $('composerAccountSheet').hidden = false;
  });
  document.querySelector('[data-close-ca-sheet]')?.addEventListener('click', () => {
    $('composerAccountSheet').hidden = true;
  });
  document.querySelector('#composerAccountSheet .sheet-backdrop')?.addEventListener('click', () => {
    $('composerAccountSheet').hidden = true;
  });
  $('composerCalendarBtn')?.addEventListener('click', () => {
    ...
    ui.renderComposerCalendarList(state, accId, (calId) => {
      const h = $('composerCalendar');
      if (h) h.value = calId;
      ui.fillComposerCalendarBtn(state, calId);
      $('composerCalendarSheet').hidden = true;
    });
    $('composerCalendarSheet').hidden = false;
  });
  document.querySelector('[data-close-cc-sheet]')?.addEventListener('click', () => {
    $('composerCalendarSheet').hidden = true;
  });
  document.querySelector('#composerCalendarSheet .sheet-backdrop')?.addEventListener('click', () => {
    $('composerCalendarSheet').hidden = true;
  });

  $('settingsModal')?.addEventListener('click', (ev) => {
    if (ev.target === $('settingsModal') || ev.target.hasAttribute?.('data-close-settings')) closeSettings();
  });
  ...
  // legacy confirm modal — keep as fallback but prefer deleteSheet
  ...
  $('confirmOk')?.addEventListener('click', async () => {
    ...
    state.pendingDelete = null;
    $('confirmModal')?.classList.remove('open');
    if (pending?.ev) await askDelete(pending.ev);
  });
  ...
  if (handle) {
    handle.addEventListener('click', () => {
      const cur = drawer.getAttribute('data-detent') || 'peek';
      if (cur === 'peek') ui.setDrawerDetent('half');
      else if (cur === 'half') ui.setDrawerDetent('full');
      else ui.setDrawerDetent('peek');
      ui.setFabVisibility(state);
    ...
    window.addEventListener(
      ...
      () => {
        ...
        drawer.style.height = '';
        if (h < vh * 0.18) clearDaySelection();
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      const ds = $('deleteSheet');
      if (ds && !ds.hidden) ds.hidden = true;
      $('confirmModal')?.classList.remove('open');
      const yms = $('ymSheet');
      if (yms && !yms.hidden) yms.hidden = true;
      const ca = $('composerAccountSheet');
      if (ca && !ca.hidden) ca.hidden = true;
      const cc = $('composerCalendarSheet');
    ...
    if ((e.key === 't' || e.key === 'T') && !e.target.closest('input,textarea,[contenteditable]'))
      $('todayBtn')?.click();
  });
}
...
async function boot() {
  ...
  maybeEnableAuth();
  ui.setChromeVisibility(state);
  // If all accounts stale on boot, show banner
  if (state.accounts.length && !liveAccounts(state).length) {
    updateConnectivity();
  }
}
