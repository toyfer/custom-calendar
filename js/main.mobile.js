...
function paint(){
  ...
  ui.setChromeVisibility(state); ui.syncThemeUI(getStoredTheme());
...
function askDelete(ev){ ui.openDeleteSheet(ev,{ onConfirm: async()=>{ try{ await apiDeleteEvent(state,ev.accountId,ev.id,ev.calendarId||'primary',{scope:'single',recurringEventId:ev.recurringEventId}); closeComposer(); ui.toast('削除しました','ok'); ui.closeDaySheet(); state.selectedDate=null; persistAccounts(state); await fetchAll({force:true}); }catch(err){ const m=err?.message||String(err); if(String(m).includes('401')||err?.status===401){ const a=accountById(state,ev.accountId); if(a) a.stale=true; persistAccounts(state); ui.toast('セッション切れ — 再連携してください','error'); paint(); openAcctSheet(); } else ui.toast('削除失敗','error'); } }}); }
